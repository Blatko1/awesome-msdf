...
float thicknessFactor = 0.4; // Range: -0.4 < thickness < 0.4
...
void main() {
	...
	float opacity = smoothstep(-0.5-thickness, 0.5-thickness, pxDist);
	
	if (pxRange < 2) {
			float opacity = smoothstep(-0.5, 0.5, (dist-0.5+thicknessFactor)*pxRange);
